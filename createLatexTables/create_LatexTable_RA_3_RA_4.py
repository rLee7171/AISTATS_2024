...
different_model_names = ["Radial$_{1,2}$", "Radial$_{2,1}$"]
    ...
    for embed in embedding_list:
        with open(f"{root_dir}/Latex_Tables/{per_name}_allDS_{embed}_RA_3_RA_4.tex", "w") as latex_table:
            ...
            if per_name == "avg_valid":
                latex_table.write(f"\\caption{{Average validation accuracy overtime on all datasets, Spectral$_{{NS}}$ embedding}}")
            else:
                latex_table.write(f"\\caption{{Average test accuracy overtime on all datasets, Spectral$_{{NS}}$ embedding}}")
            ...
            for dataset in dataset_name:
                with open(f"{root_dir}"+"/Table_Results/"+f"average_performance_{dataset}_{embed}_RA_3_RA_4.txt") as jsonFile:
                    data = json.load(jsonFile)
                    for model_index in range(len(model_name)):
                        results = []
                        for e in epochs:
                            results.append(int(round(data[model_name[model_index]][per_name][e], 2)*100))
                        latex_table.write("\\textsc{"+dataset+"-"+different_model_names[model_index]+
                                          "} & " + f"{results[0]}\\%" +
                                          " & " + f"{results[1]}\\%" +
                                          " & " + f"{results[2]}\\%" +
                                          " & " + f"{results[3]}\\%" +
