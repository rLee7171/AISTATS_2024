        ...
        for embed in embedding_list:
            ...
            for model in model_name:
                ...
                for experiment_number in range(num_experiments):
                    with open(f"{root_dir}"+"/Truncated_Results/"+f"highest_Validation_{dataset}_truncated_spectrumMLP_{model}.txt") as jsonFile:
            ...
            for record_index in range(len(all_records)):
                ...
                plt.savefig(f"{root_dir}/Truncated_Graphs/{dataset}_{record_names[record_index]}_coeff_{coeff}_embed_{embed}.png")
                plt.close()
