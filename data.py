...
def embedding(data, dataset_name, root_dir, param, ncol=0, drp_first=True, use_cache = True, mode="non-symmetric"):
    ...
    if mode in ["non-symmetric", "symmetric"]:
        ...
        cache_dir_egval = root_dir + "/Cache/eigval_embedding_" + str(dataset_name) + "_param_" + str(param) + ".pt"
        cache_dir_egvec = root_dir + "/Cache/eigvec_embedding_" + str(dataset_name) + "_param_" + str(param) + ".pt"
...
######################################################################################################################### def deepwalk
def deepwalk(data, root_dir, dataset_name, device, param,
    ...
    if use_cache:
        cache_dir = root_dir + "/Cache/deepwalk_embedding_" + str(dataset_name) + "_param_" + str(param) + ".pt"
        dw = torch.load(cache_dir, map_location=torch.device('cpu'))
        return dw

    else:
        cache_dir = root_dir + "/Cache/deepwalk_embedding_" + str(dataset_name) + "_param_" + str(param) + ".pt"
        #g = to_dgl(data.to(device))
        #g.to(device)
        g = to_dgl(data)
        ...
        for epoch in range(n_epoch):
            ...
            for batch_walk in dataloader:
                print(f"Deepwalk Epoch: {epoch}, Batch: {counter}")
                batch_walk.to(device)
                loss = mdl(batch_walk)
                print(f"Loss: {loss}")
                opt.zero_grad()
                loss.backward()
                opt.step()
                counter+=1
        torch.save(mdl.node_embed.weight.detach(), cache_dir)
