    ...
    for model_index in range(len(model_name)):
        for coeff in coeff_list:
            ...
            for embed_index in range(len(embedding_list)):
                average_performance_dic[embedding_list[embed_index]] = {}
                average_performance_dic[embedding_list[embed_index]]["avg_valid"] = {}
                average_performance_dic[embedding_list[embed_index]]["avg_test"] = {}
                avg_valid_acc = []
                avg_test_acc = []
                for experiment_number in range(num_experiments):
                    if embedding_list[embed_index] == "truncated-spectral":
                        with open(f"{root_dir}"+"/Truncated_Results/"+f"highest_Validation_{dataset}_{model_name[model_index]}_truncated_model_4.txt") as jsonFile:
                            ...
                            for e in epochs:
                                embed_validation_acc.append(data[coeff][embedding_list[embed_index]][str(experiment_number+1)][e][0])
                                embed_test_acc.append(data[coeff][embedding_list[embed_index]][str(experiment_number+1)][e][1])
                            avg_valid_acc.append(embed_validation_acc)
                            avg_test_acc.append(embed_test_acc)
                    else:
                        with open(f"{root_dir}"+"/Results/"+f"highest_Validation_{dataset}_{model_name[model_index]}.txt") as jsonFile:
                            embed_validation_acc = []
                            embed_test_acc = []
                            data = json.load(jsonFile)
                            for e in epochs:
                ...
                for i in range(len(avg_valid_acc)):
                    if tracker % 10 == 0:
                        average_performance_dic[embedding_list[embed_index]]["avg_valid"][tracker] = avg_valid_acc[i]
                    tracker+=5
                tracker = 5
                for i in range(len(avg_test_acc)):
                    if tracker % 10 == 0:
                        average_performance_dic[embedding_list[embed_index]]["avg_test"][tracker] = avg_test_acc[i]
                    tracker+=5
            ...
            font_size = 15
            with open(f"{root_dir}/Table_Results/average_performance_{dataset}_model_{combined_name}.txt", "w") as fp:
